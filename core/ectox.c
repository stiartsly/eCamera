    ...
    while(!node->threadQuit) {
        ...
        if (msg->msgId > 0) {
            ...
            case msgFriendRequest:
                printf("cb::msgFriendRequest\n");
                if (node->handler && node->handler->onFriendAddRequest) {
                    node->handler->onFriendAddRequest(msg->key, msg->data, msg->length, msg->cookie);
                }
                free(msg->data);
                msg->msgId = 0;
                break;
            case msgFriendMessage:
                printf("cb::msgFriendMessage\n");
                if (node->handler && node->handler->onFriendMessage) {
                    node->handler->onFriendMessage(msg->friendId, msg->data, msg->length, msg->cookie);
                }
                free(msg->data);
                msg->msgId = 0;
                break;
            case msgFriendConnectionMessage:
                printf("cb::msgFriendConnection\n");
                if (msg->status == TOX_CONNECTION_NONE) {
                    if (node->handler && node->handler->onFriendDisconnected) {
                        node->handler->onFriendDisconnected(msg->friendId, msg->cookie);
                    }
                } else {
                    if (node->handler && node->handler->onFriendConnected) {
                        node->handler->onFriendConnected(msg->friendId, msg->cookie);
                    }
                }
                msg->msgId = 0;
                break;
            case msgFriendLossyPacket:
                printf("cb::msgFriendLossyPacket\n");
                if (node->handler && node->handler->onLossyPacketReceived) {
                    node->handler->onLosslessPacketReceived(msg->friendId, msg->data, msg->length, msg->cookie);
                }
                free(msg->data);
                msg->msgId = 0;
                break;
            case msgFriendLosslessPacket:
                printf("cb::msgFriendLosslessPacket\n");
...
int32_t _auxAvThreadEntry(void* params)
{
    ...
    while(!avNode->threadQuit) {
        ...
        Sleep(toxav_iteration_interval(avNode->av));
    }
    return 0;
}
