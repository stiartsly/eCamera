...
void _friendReqCb(Tox* tox, const uint8_t* id, const uint8_t* msg, size_t length, void* userData)
{
    ...
    printf("Friend adding request\n");

    ToxMsg* tmsg = &node->msg;
    tmsg->data = calloc(1, length);
    if (!tmsg->data) {
         printf("Error: memory calloc failed\n");
         return;
    }
    memcpy(tmsg->data, msg, length);
    memcpy(tmsg->key,  id,  TOX_PUBLIC_KEY_SIZE);
    tmsg->msgId  = msgFriendRequest;
    tmsg->length = length;
    tmsg->cookie = node->userData;
    return;
}
...
void _friendConnectionCb(Tox* tox, uint32_t fid, TOX_CONNECTION status, void* userData)
{
     ...
     msg->msgId    = msgFriendConnectionMessage;
     msg->friendId = fid;
     msg->status   = status;
     msg->cookie   = node->userData;
...
{
    printf("[DHT] received a lossy packet.\n");
    ...
    msg->length = length;
    msg->msgId  = msgFriendLossyPacket;
    msg->friendId = fid;
    msg->cookie = node->userData;
    return;
}
...
{
    printf("[DHT] received a lossless packet.\n");
    ...
    memcpy(msg->data, data, length);
    msg->length = length;
    msg->msgId  = msgFriendLosslessPacket;
    msg->friendId = fid;
    msg->cookie = node->userData;
    return;
}
